<div class="py-16 lg:py-28 space-y-12">
    ...
    <div class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-8">
        <div class="space-y-3">
            <flux:heading size="2xl" class="leading-7 tracking-tight text-white">{{ __('User Management') }}</flux:heading>
            <flux:subheading class="leading-6 text-white/80 max-w-xl">{{ __('Manage your application users and roles here.') }}</flux:subheading>
            <div class="flex flex-wrap items-center gap-3 pt-2">
                ...
                <span class="inline-flex items-center rounded-full bg-emerald-500 text-white px-3 py-1 text-xs font-medium leading-none">{{ __(':count active', ['count' => $activeCount]) }}</span>
            </div>
        </div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-6 py-6 border-y border-white/10">
        <div class="w-full sm:w-[420px] relative">
            ...
            <div wire:loading wire:target="search, roleFilter, statusFilter, sort" class="absolute right-3 top-1/2 -translate-y-1/2">
                <flux:icon icon="arrow-path" class="animate-spin size-3.5 text-white/40" />
            </div>
        </div>
    ...
    @if ($users->isEmpty())
        <div class="bg-[#0f0f0f] rounded-[var(--radius-lg)] border border-white/10 p-16 lg:p-24 text-center">
            <flux:heading size="lg" class="leading-7 text-white">{{ __('No users found') }}</flux:heading>
            <flux:text class="mt-3 leading-6 text-white/60">
    ...
    @else
        ...
        <div class="bg-[#0f0f0f] rounded-[var(--radius-lg)] overflow-hidden border border-white/10">
            <flux:table :paginate="$users" class="w-full">
                <flux:table.columns class="bg-white/[0.02]">
                ...
                <flux:table.rows>
                    @foreach ($users as $user)
                        <flux:table.row :key="$user->id" class="hover:bg-white/[0.04] transition-colors">
                            <flux:table.cell>
                                <flux:avatar
                                    circle
                                    size="lg"
                                    ...
                                    :src="$user->avatarUrl()"
                                    class="h-12 w-12 text-sm leading-none ring-1 ring-white/10"
                                />
                            </flux:table.cell>
                            <flux:table.cell class="font-medium leading-6">
                                @can('view', $user)
                                    <flux:link href="{{ route(config('user-management.routes.names.users.show', 'admin.users.show'), $user) }}" wire:navigate class="leading-6 text-white hover:text-white/80">{{ $user->name }}</flux:link>
                                @else
                                    <span class="leading-6 text-white">{{ $user->name }}</span>
                                @endcan
                            </flux:table.cell>
                            <flux:table.cell>
                                <flux:text class="text-sm leading-6 text-white/60">{{ $user->email }}</flux:text>
                            </flux:table.cell>
                            <flux:table.cell>
                                <div class="flex flex-wrap gap-2">
                                    @forelse ($user->roles as $role)
                                        @php($enum = \Kreetancraft\UserManagement\Enums\UserRole::tryFrom($role->name))
                                        <span class="inline-flex items-center rounded-full bg-white/10 text-white px-2.5 py-1 text-xs font-medium leading-none ring-1 ring-white/10">
                                            {{ $enum?->label() ?? $role->name }}
                                        </span>
                                    @empty
                                        <flux:text class="text-xs leading-5 text-white/40">{{ __('No Roles') }}</flux:text>
                                    @endforelse
                                </div>
                            </flux:table.cell>
                            <flux:table.cell>
                                @if ($user->is_active)
                                    <span class="inline-flex items-center rounded-full bg-emerald-500/30 text-emerald-200 px-2.5 py-1 text-xs font-medium leading-none ring-1 ring-emerald-500/20">{{ __('Active') }}</span>
                                @else
                                    <span class="inline-flex items-center rounded-full bg-white/10 text-white/60 px-2.5 py-1 text-xs font-medium leading-none">{{ __('Inactive') }}</span>
                                @endif
                            </flux:table.cell>
                            <flux:table.cell>
                                @if ($user->last_login_at)
                                    <flux:text class="text-sm leading-6 text-white/60">{{ $user->last_login_at->diffForHumans() }}</flux:text>
                                @else
                                    <flux:text class="text-sm leading-6 text-white/40">{{ __('Never') }}</flux:text>
                                @endif
                            </flux:table.cell>
                            <flux:table.cell class="text-right">
                                <flux:dropdown>
                                    ...
                                    <flux:menu class="min-w-[200px] bg-[#0f0f0f] border border-white/10 rounded-[var(--radius-lg)]">
                                        ...
                                        @can('delete', $user)
                                            ...
                                            <flux:menu.item class="leading-5 text-red-300 hover:text-red-200" wire:click="confirmDelete({{ $user->id }})" icon="trash" variant="danger" data-test="delete-user-{{ $user->id }}">{{ __('Delete') }}</flux:menu.item>
                                        @endcan
                                    </flux:menu>
                                </flux:dropdown>
                            </flux:table.cell>
            ...
            <div class="px-6 py-4 border-t border-white/10">
                <flux:text class="text-sm leading-6 text-white/60">{{ __('Showing :from to :to of :total results', ['from' => $users->firstItem(), 'to' => $users->lastItem(), 'total' => $users->total()]) }}</flux:text>
            </div>
        </div>
    @endif
    ...
    <flux:modal name="confirm-delete-user" class="md:w-96">
        <div class="space-y-6 p-2">
            <flux:heading size="lg" class="leading-7 text-white">{{ __('Delete User?') }}</flux:heading>
            <flux:text class="leading-6 text-white/60">{{ __('This action cannot be undone. The user will be permanently removed from the system.') }}</flux:text>
